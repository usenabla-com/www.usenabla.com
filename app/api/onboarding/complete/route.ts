...
export async function POST(req: NextRequest) {
  try {
    ...
    const { plan, name, org, email, posthog_id, session_id } = body || {};
    if (!plan || !email || !org) {
    ...
    if (!dbUrl) {
      return NextResponse.json({ error: "Missing NEON_DATABASE_URL" }, { status: 500 });
    }
    ...
    const displayName = (name && String(name).trim()) || (org && String(org).trim()) || (email && String(email).trim()) || "New Customer";

    // Insert into customers table (stytch_* columns for identity)
    await sql`
        ...
        await resend.emails.send({
          ...
          html: renderEmailFrame({
            ...
            contentHtml: `
              ...
              <h1 style=\"margin:0 0 12px;font-size:20px;line-height:1.35;color:#0f172a;\">Welcome — your 14‑day ${escapeHtml(
                plan
              )} trial is active</h1>
              <p style=\"margin:0 0 16px;font-size:14px;color:#334155;\">Use the API key below to authenticate requests to Nabla. Keep it secret. You can rotate it at any time.</p>
              ...
              <p style=\"margin:18px 0 0;font-size:13px;color:#334155;\">Questions? Just reply to this email or reach us at <a href=\"mailto:trial@usenabla.com\" style=\"color:#0f172a;text-decoration:underline;\">trial@usenabla.com</a>.</p>
              <p style=\"margin:6px 0 0;font-size:12px;color:#64748b;\">— The Nabla Team</p>
            `,
            footerNote: `You’re receiving this because a ${escapeHtml(
              plan
            )} trial was initiated for ${escapeHtml(org || "your organization")}.`,
          }),
        });
      }
    } catch (e) {
        ...
        await resend.emails.send({
          ...
          html: renderEmailFrame({
                ...
                contentHtml: `
                  ...
                  <h1 style=\"margin:0 0 12px;font-size:20px;line-height:1.35;color:#0f172a;\">Your ${escapeHtml(
                    planLabel
                  )} trial just ended</h1>
                  <p style=\"margin:0 0 16px;font-size:14px;color:#334155;\">Let’s schedule a quick onboarding call to review outcomes and discuss next steps.</p>
